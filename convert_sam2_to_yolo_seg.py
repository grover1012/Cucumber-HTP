#!/usr/bin/env python3
"""
Convert SAM2 annotations (COCO JSON) to YOLO segmentation format with actual masks
"""
...
def convert_sam2_to_yolo_seg():
    print("🔄 CONVERTING SAM2 TO YOLO SEGMENTATION FORMAT")
    print("=" * 60)
    
    # Source and destination paths
    sam2_dir = Path("data/sam2_annotations")
    output_dir = Path("data/sam2_yolo_seg_dataset")
    ...
    for split in ["train", "valid", "test"]:
        ...
        for json_file, data in files_with_annotations:
            try:
                ...
                yolo_annotations = convert_annotations_to_yolo_seg(data, image_width, image_height)
    ...
    print(f"  2. Start training with the new dataset")

def convert_annotations_to_yolo_seg(data, img_width, img_height):
    """Convert COCO annotations to YOLO segmentation format with actual masks"""
    annotations = data.get('annotations', [])
    yolo_annotations = []
    ...
    for ann in annotations:
        try:
            ...
            # Decode RLE mask to get polygon points
            try:
                ...
                # Convert contour to normalized polygon points
                polygon_points = []
                ...
                print(f"      ⚠️  Error processing mask: {e}")
                continue
...
# Converted from SAM2 annotations with segmentation masks
...
# Format: YOLO segmentation with polygon masks
"""
